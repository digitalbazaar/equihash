...
exports.solve = (seed, options, callback) => {
  ...
  const personal =
    ...
    Buffer.alloc(exports.PERSONALBYTES, 0);
  ...
  if(personal.length > exports.PERSONALBYTES) {
    return callback(
      new Error('Equihash \'personal\' length too large.'));
  }
  ...
  if('nonceLength' in options) {
    if(options.nonceLength !== nonce.length) {
      return callback(
        new Error('Equihash \'nonce\' length and \'nonceLength\' differ.'));
    }
  }
...
exports.verify = (seed, options, callback) => {
  ...
  const k = 'k' in options ? options.k : 5;
  const nonceLength = 'nonceLength' in options ? options.nonceLength : 4;
  const personal = 'personal' in options ? options.personal : Buffer.alloc(exports.PERSONALBYTES, 0);
